 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 0; t < N; t++) {
      ...
      fg[0] += vref_cost_weight * CppAD::pow(vars[v_start + t] - ref_v, 2);
    }
    ...
    for (int t = 1; t < N; t++) {
      ...
      fg[1 + psi_start + t] = psi1 - (psi0 + v0 / Lf * delta0 * dt); // watch out for sign
      fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
      fg[1 + cte_start + t] = cte1 - (f0 - y0 + (v0 * CppAD::sin(epsi0) * dt));
      fg[1 + epsi_start + t] = epsi1 - (psi0 - psides0 + (v0 / Lf * delta0 * dt)); // watch out for sign
    }
  }
};
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  // 1 constraint for each variable
  ...
  // Initial state indices lower and upper bound
